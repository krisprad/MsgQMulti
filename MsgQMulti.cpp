// MsgQMulti.cpp: multiple message Q-s
...
class Consumer
{
	...
	std::thread m_thread; // default constructed thread
	unique_ptr<Consumer> m_nextConsumer; // next consumer
...
public:
	...
	void setNextConsumer(unique_ptr<Consumer> nextConsumer) { m_nextConsumer = nextConsumer; }
	void SetName(const char* s_) { m_name = s_; }
	const string& GetName() const { return m_name; }
	...
	void Run()
	{
		...
		while (!m_stop)
		{
			...
			ObjType* arr = (ObjType*)& m_buffer[row][0];
			size_t col = 0;
			for (; (col < m_buffer.NumColumns()) && (!m_stop); ++col)
			{
				curObj = arr[col];
				...
				prevObj = curObj;
				m_lastObj = curObj;
				lastLoc = absRow * m_buffer.NumColumns() + col; // consumed until this loc, if buffer is infinite 1-d
			...
			lastAbsRow = absRow;
			if (!m_nextConsumer) // no more consumers
...
void RunProducersConsumers(int numProd_, int numCons_, TBuffer& buffer_)
{
	...
	cout << 10 * log(buffer_.BufElemSize()) + 1
		<< " (row size: " << buffer_.BufElemSize() << "), " << usecPerProd * 100 << endl;

	// uncomment this for more stats
	/*
	...
	cout << "------Number of consumers : " << numCons_ << ", Total consumed "
		 ...
		 */
	cout << "Last produced " << lastProduced << ", last consumed " << lastConsumed << endl;
...
int main(int argc, char** argv)
{
	...
	static constexpr size_t BufSize = 10000000;
	static constexpr size_t NumRows = 1000; // sqrt of BufSize
	static constexpr size_t NumColumns = 1000; // sqrt of BufSize
	typedef MBuffer<NumRows, NumColumns, ObjType<int64_t>> BufType;
